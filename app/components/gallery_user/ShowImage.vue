...
const enableMediumRes = ref(false)
const mediumImagePath = computed(() => enableMediumRes.value ? `${imagePath.value}?mode=medium` : '')
const mediumImageOptions = computed(() => ({ src: mediumImagePath.value }))
const { isLoading: mediumImageLoading, isReady: isReadyMediumImage } = useImage(mediumImageOptions)

const enableFullRes = ref(false)
const originalImagePath = computed(() => enableFullRes.value ? `${imagePath.value}?mode=original` : '')
const origImageOptions = computed(() => ({ src: originalImagePath.value }))
...
const srcToUse = computed(() => {
  if (!origImageLoading.value && isReadyOrigImage.value) {
    return originalImagePath.value
  }
  else if (!mediumImageLoading.value && isReadyMediumImage.value) {
    return mediumImagePath.value
  }
