...
import ShowImageLazy from '@/components/gallery_user/ShowImageLazy.vue'
import UploadMultipleImages from '@/components/gallery_user/UploadMultipleImages.vue'
import UploadSingleImage from '@/components/gallery_user/UploadSingleImage.vue'
import { breakpointsTailwind, useBreakpoints } from '@vueuse/core'

interface Props { imagePaths: string[], count: number, total: number, loadingMore: boolean, isLoading: boolean }
const props = defineProps<Props>()
const emit = defineEmits(['isReady'])
const { imagePaths, count, total, loadingMore, isLoading } = toRefs(props)
const { t } = useI18n()
    ...
    <Card v-if="total > 0 || isLoading">
      <template #content>
        ...
        <div class="flex flex-col items-center gap-8">
          ...
          <div v-if="loadingMore || (!allImagesReady && imagePaths?.length > 0)" class="flex flex-col gap-2 items-center justify-center">
