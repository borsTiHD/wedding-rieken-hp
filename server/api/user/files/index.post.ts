...
export default defineEventHandler(async (event) => {
  ...
  // Also set the user ID in the path
  const pathRaw = formData.find(field => field?.name === 'path')
  const pathRawString = pathRaw?.data ? `/${pathRaw?.data?.toString()}` : ''
  const pathUser = `/user/${user.uid}`
  const path = `${pathUser}${pathRawString}`
  ...
  // Upload file
  await uploadFile(fileName, fileFullPath, mimeType, fileBuffer).catch((error) => {
